 ...
 *
 * <p><b>Le point que cette preuve de concept doit démontrer.</b> En configuration de production, le
 * modèle de messagerie est branche sur un <b>relais vers un broker externe</b> (RabbitMQ, DA-11) :
 * la trame publiée ici sort de l'instance, atteint le broker, et revient vers <i>toutes</i> les
 * instances abonnées à la destination. Sans lui, le broker intégré au framework vit en mémoire
 * d'instance : un client connecté à l'instance 1 et un agent connecté à l'instance 3 ne se verraient
 * jamais, et US-24 serait inapplicable dès la seconde instance.
 *
...
public class StompChatEventPublisher implements ChatEventPublisher {
    ...
    /**
     * Destination d'une conversation : ses deux participants y sont abonnés.
     *
     * <p>Le séparateur est un point, et non une barre oblique. Ce n'est pas une coquetterie : le
     * broker interprete {@code /topic/<clé>} comme une clé de routage unique, et refuse une
     * destination comportant un second niveau de chemin. C'est le genre de contrainte qu'une
     * démonstration sur broker en mémoire ne révèle jamais — et qui apparaît au premier
     * déploiement réel.
     */
    public static final String CONVERSATION_DESTINATION = "/topic/conversations.";

    /** Destination de la file d'attente : tous les agents connectés y sont abonnés (US-26). */
    ...
    public void publish(ChatEvent event) {
        ChatEventPayload payload = ChatEventPayload.from(event);
        ...
        log.debug(
                "Événement {} diffusé pour la conversation {}",
                payload.type(),
                event.conversationId());
    }

    /**
     * Trois événements modifient la file d'attente : une demande y entre, un agent la prend — et
     * elle doit alors disparaitre des autres postes (US-26) —, ou elle est clôturée.
     */
